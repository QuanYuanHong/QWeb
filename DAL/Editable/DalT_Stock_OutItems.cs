...
namespace Book.DAL
{
    ...
    {
        public List<Book.Model.T_Stock_OutItems> GetListOfChild(string strWhere)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * ");
            strSql.Append(" FROM V_OutItems_Book ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
            DataSet ds = DbHelperSQL.Query(strSql.ToString());
            List<Book.Model.T_Stock_OutItems> lst = new List<Model.T_Stock_OutItems>();
            foreach (DataRow dr in ds.Tables[0].Rows)
            {
                Book.Model.T_Stock_OutItems item = DataRowToModel(dr);
